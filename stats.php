...
$error = 0;
$msg = "";
$o_query = $conn->prepare("SELECT COUNT(`id`) AS `organization_count` FROM `organization`");
$o_result = $o_query->execute();
$o_rows = $o_query->fetchALL();

$v_query = $conn->prepare("SELECT COUNT(`id`) as `vaccine_count` FROM `vaccine`");
$v_result = $v_query->execute();
$v_rows = $v_query->fetchALL();
